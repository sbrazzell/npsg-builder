...
export default async function ProjectsPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const totalBudget = facility.projectProposals.reduce((sum, p) =>
    sum + p.budgetItems.reduce((s, b) => s + b.totalCost, 0), 0)
        ...
        {facility.projectProposals.length === 0 ? (
          ...
          <div className="space-y-3">
            {facility.projectProposals.map((project) => {
              const budget = project.budgetItems.reduce((s, b) => s + b.totalCost, 0)
              return (
