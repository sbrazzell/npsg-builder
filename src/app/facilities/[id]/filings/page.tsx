    ...
    <div>
      ...
      <div className="p-4 md:p-8 max-w-4xl">
        <PageHeader
          ...
          action={<CreateDraftButton facilityId={id} nextVersion={nextVersion} />}
        />
        ...
        {finalDraft && (
          <div className="mb-6 rounded-lg border border-emerald-200 bg-emerald-50 px-4 py-3 flex items-center gap-3">
            ...
            <Button asChild size="sm" variant="outline" className="shrink-0 border-emerald-300 text-emerald-800">
              <Link href={`/facilities/${id}/filings/${finalDraft.id}`}>
        ...
        {drafts.length === 0 ? (
          <div className="text-center py-16">
            ...
            <CreateDraftButton facilityId={id} nextVersion={1} />
          </div>
        ) : (
                    ...
                    <div className="flex items-start gap-4">
                      ...
                      <div className="shrink-0 flex items-center gap-2">
                        <Button asChild size="sm" variant="ghost">
                          <Link href={`/facilities/${id}/filings/${draft.id}`}>
                            View Forms <ChevronRight className="h-3.5 w-3.5 ml-1" />
                          </Link>
                        </Button>
                        <DraftActions draftId={draft.id} facilityId={id} status={draft.status} />
                      </div>
                    </div>
                  </CardContent>
                </Card>
