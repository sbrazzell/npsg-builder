...
export function FormSF424({ snapshot }: { snapshot: FilingSnapshot }) {
  const { organization: org, facility, projects, totalBudget } = snapshot
      ...
      <div className="border-2 border-gray-800 mb-0">
        ...
        <div className="p-4 grid grid-cols-2 gap-0.5 bg-gray-200">
          ...
          <Field
            ...
            value={`Security enhancements for ${facility.facilityName}: ${projectTitles || 'Physical security improvements'}`}
            wide
          />
          <Field label="Project Location — Facility Name" value={facility.facilityName} />
          <Field label="Project Location — Address" value={facility.address || org.address || ''} />
