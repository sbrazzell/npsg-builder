...
export default async function DraftPage({
  ...
  const [facility, draft] = await Promise.all([
    prisma.facility.findUnique({ where: { id }, include: { organization: true } }),
    prisma.applicationDraft.findUnique({ where: { id: draftId } }),
  ])

  if (!facility || !draft || draft.facilityId !== id) notFound()
  ...
  return (
    <div>
      <Header breadcrumbs={[
        ...
        { label: 'Filings', href: `/facilities/${id}/filings` },
        { label: draft.title },
      ]} />
          ...
          <div>
            <div className="flex items-center gap-3 flex-wrap">
              <Button asChild size="sm" variant="ghost" className="-ml-2 text-muted-foreground">
                <Link href={`/facilities/${id}/filings`}>
