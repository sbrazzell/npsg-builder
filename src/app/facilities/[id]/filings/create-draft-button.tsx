...
export function CreateDraftButton({ facilityId, nextVersion }: { facilityId: string; nextVersion: number }) {
  ...
  async function handleCreate() {
    ...
    setLoading(true)
    const result = await createDraft(facilityId, title.trim(), notes.trim() || undefined)
    if (result.success && result.data) {
      toast.success(`Draft v${nextVersion} saved`)
      setOpen(false)
      router.push(`/facilities/${facilityId}/filings/${result.data.id}`)
    } else {
