...
export default async function FacilityPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const navItems = [
    { label: 'Threat Assessments', href: `/sites/${id}/threats`, icon: AlertTriangle, count: facility.threatAssessments.length, color: 'text-red-600', bg: 'bg-red-50' },
    { label: 'Security Measures', href: `/sites/${id}/measures`, icon: Shield, count: facility.securityMeasures.length, color: 'text-blue-600', bg: 'bg-blue-50' },
    { label: 'Projects', href: `/sites/${id}/projects`, icon: FileText, count: facility.projectProposals.length, color: 'text-violet-600', bg: 'bg-violet-50' },
    { label: 'Narratives', href: `/sites/${id}/narratives`, icon: MessageSquare, count: facility.narrativeDrafts.length, color: 'text-emerald-600', bg: 'bg-emerald-50' },
    { label: 'Site Observations', href: `/sites/${id}/observations`, icon: Eye, count: facility.siteObservations.length, color: 'text-amber-600', bg: 'bg-amber-50' },
    { label: 'Filings & Forms', href: `/sites/${id}/filings`, icon: FolderOpen, count: (facility as any).applicationDrafts?.length ?? 0, color: finalDraft ? 'text-emerald-600' : 'text-indigo-600', bg: finalDraft ? 'bg-emerald-50' : 'bg-indigo-50' },
    { label: 'Review & Scorecard', href: `/sites/${id}/review`, icon: ClipboardCheck, count: null, color: 'text-slate-600', bg: 'bg-slate-50' },
    { label: 'Export', href: `/sites/${id}/export`, icon: Download, count: null, color: 'text-gray-600', bg: 'bg-gray-50' },
  ]
        ...
        {!(facility as any).lawEnforcementAgency ? (
          <div className="mb-6 rounded-lg border border-amber-200 bg-amber-50 px-4 py-3 flex items-start gap-3">
            ...
            <Button asChild size="sm" variant="outline" className="shrink-0 border-amber-300 text-amber-800 hover:bg-amber-100">
              <Link href={`/sites/${id}/edit#law-enforcement`}>Log Outreach</Link>
            </Button>
          </div>
        ) : (
          <div className="mb-6 rounded-lg border border-blue-200 bg-blue-50 px-4 py-3 flex items-start gap-3">
            ...
            <Button asChild size="sm" variant="ghost" className="shrink-0 text-blue-700">
              <Link href={`/sites/${id}/edit`}>Edit</Link>
            </Button>
          </div>
        )}
          ...
          <Card>
            <CardHeader className="flex flex-row items-center justify-between pb-2">
              ...
              <Button asChild size="sm" variant="ghost">
                <Link href={`/sites/${id}/threats`}>View all</Link>
              </Button>
            </CardHeader>
            <CardContent>
              {facility.threatAssessments.length === 0 ? (
                <div className="text-center py-4">
                  ...
                  <Button asChild size="sm" variant="outline">
                    <Link href={`/sites/${id}/threats/new`}>
          ...
          {facility.projectProposals.length > 0 && (
            <Card className="lg:col-span-2">
              <CardHeader className="flex flex-row items-center justify-between pb-2">
                ...
                <Button asChild size="sm" variant="ghost">
                  <Link href={`/sites/${id}/projects`}>View all</Link>
                </Button>
              </CardHeader>
              <CardContent>
