...
export function FormInvestmentJustification({ snapshot }: { snapshot: FilingSnapshot }) {
  const { organization: org, site, threats, securityMeasures, projects, narratives, totalBudget } = snapshot
  ...
  const leReceived = !!(site as any).lawEnforcementResponseDate
    ...
    <div className="font-sans text-gray-900 bg-white" id="form-ij">
      ...
      <div className="px-8 pb-8 space-y-0">
        ...
        <IjSection number="2" title="Vulnerability & Threat Assessment">
          ...
          {(site as any).lawEnforcementAgency && (
            <div className={`mb-4 rounded border px-4 py-3 ${leReceived ? 'border-blue-200 bg-blue-50' : 'border-amber-200 bg-amber-50'}`}>
              ...
              <p className="text-sm">
                <strong>{(site as any).lawEnforcementAgency}</strong>
                {(site as any).lawEnforcementContactName && ` — ${(site as any).lawEnforcementContactName}`}
                {leReceived
                  ? ` · Assessment received ${new Date((site as any).lawEnforcementResponseDate).toLocaleDateString()}`
                  : ` · Requested ${(site as any).lawEnforcementContactDate ? new Date((site as any).lawEnforcementContactDate).toLocaleDateString() : ''} — pending response`}
              </p>
              {(site as any).lawEnforcementFindings && (
                <p className="text-sm mt-2 text-slate-700 italic">{(site as any).lawEnforcementFindings}</p>
              )}
            </div>
          )}
