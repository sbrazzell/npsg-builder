...
export function ApplyRewriteButton({ siteId, sectionName, rewriteText }: ApplyRewriteButtonProps) {
  const [isPending, startTransition] = useTransition()
  const [applied, setApplied] = useState(false)

  function handleApply() {
    startTransition(async () => {
      const result = await applyGeneratedRewrite(siteId, sectionName, rewriteText)
